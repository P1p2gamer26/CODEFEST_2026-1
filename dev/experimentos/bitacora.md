...
documentado en `dev/docs/PLAN_MAESTRO.md`): no era su tamano ni su ventana, es que **la
...
medicion independiente de la nota de dev/docs/PLAN_MAESTRO.md ("la prioridad de idioma no es
...
---
## E41 - diagnostico: E32 exonerado como causa de los 11 ceros (11 ago 2026)

Arnes fiel (replica del camino online: base = MiniLM + 0.6*gte + 0.6*e5 ->
top100 -> filtro de fenomeno por voto de score, umbral 0.8 -> agregacion top5
-> top3) reproduce 
esultados.jsonl byte a byte en los 50 top-3 y 0.4547 de
F1 (0.455 oficial), con los 11 ceros identicos. Diagnostico de causa de los
ceros.

**Hipotesis cerrada:** el post-filtrado por fenomeno (E32) era sospechoso de
ser la causa de los ceros, porque ambos nacieron el mismo dia. **Falsa.** Los
11 ceros reales (F1=0, eval_mini.py): q003, q005, q007, q011, q014, q015,
q034, q037, q044, q046, q047. E32 queda exonerado por construccion:

- En q003, q011, q015, q005, q007 el voto ni supera el umbral (dominancia
  0.50-0.61) -> el filtro se abstiene y no toca nada.
- En q034, q037, q044, q046, q047 filtra, pero el fenomeno que elige ES el
  del documento relevante (F3 en las cinco, dominancia 0.86-1.00). No
  descarta ningun relevante: estos mueren DESPUES, en la agregacion top5.

**Los ceros son de ranking, no de pool ciego.** q001 y q028 (los que este
proyecto creia irrecuperables por NBQR/CBRN y on-orbit servicing) tienen hoy
F1 0.40 y 0.67: el glosario + el rescate los trajeron y no son ceros. De los
11 reales, los relevantes de q005/q007/q014/q015 estan en el top-200 del
primario pero fuera del top-100 de agregacion (por eso E18 los veia en el
pool); los de q003/q011/q034/q037/q044/q046/q047 estan en el top-100 y mueren
en la agregacion top5.

**El mecanismo real es el que E19/E20 ya habian medido, ahora con arnes
fiel:** el relevante tiene el MEJOR chunk del pool (top1 score 1.667-1.751,
por encima del ganador) pero aporta pocos chunks (2-9), y el ganador aporta
muchos (8-38). Con top5, el documento con 30 chunks satura el tope y el
relevante con 3-6 suma menos aunque su mejor chunk valga mas. Los margenes
por consulta (relevant@agg vs agg3): q037 7.6414 vs 7.6501 (-0.0087), q047
5.8903 vs 5.9011 (-0.0108), q003 8.3061 vs 8.3792 (-0.073). El resto cae a
agg8-26, muy lejos del cupo.

**Techo de la agregacion:** si los relevantes rank4-6 entraran al top-3, el
F1(50) subiria de 0.504 a 0.690 (+8.0) y 5 de los 11 ceros serian salvables.
Pero ninguna celda de agregacion los salva sin disparar el veto: top1/max
rescata q007/q011/q034/q044/q047 pero crea 23 ceros (12 nuevos); top3 0.4053
con 12; top7/top8/sum 0.4047 con 13; mean 0.0567 con 43.

**Hallazgo de metodo:** la metrica oficial F1@3 es armonica (P@3 con recall
denominador min(|D*|,3)), no aciertos/3 -- eval_mini.py:58. En 43 de 50
consultas |D*|>=3 y da igual; en las 7 con menos el denominador es |D*|. Por
eso la suma ponderada por consulta de un barrido no coincide con el F1
global y la grilla hay que evaluarla con eval_mini.py real.

**Veredicto:** E32 queda exonerado; el eje de agregacion sigue cerrado por
E19/E20/E33, y este diagnostico lo confirma sin abrirlo. No reabrir.

## E42 — normalizacion por tamano de documento en la agregacion (12 ago 2026)

**Hipotesis:** dividir el score agregado por documento por el numero de
chunks que aporta al pool (o al corpus), elevado a un exponente pequeno
alpha, penaliza al documento que inunda el pool con muchos chunks mediocres
sin castigar al que aporta pocos chunks buenos.

**Justificacion mecanica (escrita antes de medir):** E41 midio con arnes
fiel que los 11 ceros no son de pool ciego sino de ranking: en
q003/q011/q034/q037/q044/q046/q047 el documento relevante esta dentro del
top-100 y tiene el mejor chunk del pool (score top1 1.667-1.751, por encima
del ganador), pero aporta 2-9 chunks mientras el ganador aporta 8-38. Con
top5 el ganador satura el tope de cinco sumandos y el relevante no llega.
Los margenes son minusculos: q037 7.6414 vs 7.6501 (-0.0087), q047 5.8903 vs
5.9011 (-0.0108). No es el eje de topM (E07/E19/E20/E33/E37 cerrado, topM es
un tope y ya esta saturado en los dos documentos); esto es un normalizador.
Riesgo declarado antes de medir: un alpha grande degenera en mean, que E41
midio en 0.0567 con 43 ceros -- la grilla se pre-registro pegada a cero y no
se extiende hacia arriba aunque el borde superior gane.

Grilla pre-registrada y cerrada: alpha en {0.00, 0.02, 0.05, 0.10, 0.20} x
denominador en {n_pool, n_corpus}. Arnes sin FAISS, sobre
`dev/intermedios/pools_entregados.json`. Puerta de fidelidad: la celda base
reproduce **50 de 50** top-3 de `Entrega/resultados.jsonl`.

**Bug de arnes corregido antes de leer resultados:** el `break` del brief
dentro del loop de alphas cortaba TODA la grilla del segundo denominador, no
solo la repeticion de la celda base; corregido a `continue` condicionado.
...
**IC al 90% del delta pareado de NDCG@10 contra la base (las que no caen por
el veto):** n_pool:a0.02 -0.0011 [-0.0248, +0.0232] (no pasa: limite inferior
< -0.02); n_pool:a0.05 -0.0243 [-0.0655, +0.0136] (no pasa); n_pool:a0.10
-0.0318 [-0.0858, +0.0169] (no pasa); n_corpus:a0.05 -0.0014 [-0.0456,
+0.0418] (no pasa); **n_corpus:a0.02 +0.0197 [-0.0080, +0.0491] en las 50 y
+0.0000 [+0.0000, +0.0000] en las 10 independientes -- pasa en las dos
muestras.**

**Veredicto: ADOPTABLE, y por la regla del Step 8 (veto, luego IC en las dos
muestras, luego alpha mas chico entre las que sobreviven) la celda unica es
`n_corpus:a0.02`.** F1(50) 0.4547 -> 0.4740, ND(50) 0.5149 -> 0.5345, ceros 11
-> 9. En las 10 independientes el delta es exactamente 0.0000 en F1 y en
NDCG: ninguna de las 10 consultas independientes cambia de documentos con
esta variante -- no confirma ganancia ahi, pero tampoco pierde nada.
...
`generador.py` (y decidir el coste de leer `metadata.jsonl` completo para
