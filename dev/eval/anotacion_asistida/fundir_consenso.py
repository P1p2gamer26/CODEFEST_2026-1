#!/usr/bin/env python3
"""Funde el consenso del la anotacion asistida en ground_truth_mini.jsonl.
...
Uso:
    python dev/eval/anotacion_asistida/fundir_consenso.py            # simulacion
    python dev/eval/anotacion_asistida/fundir_consenso.py --escribir
"""
...
VOTO_MINIMO = 2
# Las 9 que ningun humano pudo marcar. La anotacion asistida solo puede tocar estas: pisar
# una etiqueta humana con una asistida seria cambiar el patron de medida.
...
def main():
    ...
    for qid in sorted(OBJETIVO):
        ...
        por_qid[qid] = {
            "query_id": qid,
            "nota": f"consenso de {len(SESGOS)} rondas de anotacion asistida, >={args.voto_minimo} votos",
            "anotador": "anotacion-asistida",
    ...
    print(f"escrito {GT}: {len(por_qid)} consultas ({humanas} humanas + {nuevas} asistidas)")
