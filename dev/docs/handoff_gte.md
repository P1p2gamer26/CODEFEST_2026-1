...
> índice. El estado vigente está en `CLAUDE.md`, sección "Adoptado:
...
3. **Re-aplanar** cualquier cambio del camino online en `generador.py`
   (punto 14 de `CLAUDE.md`: es autocontenido).
...
5. Publicar el índice por **GitHub Release**, no por LFS (punto 16 de
   `CLAUDE.md`: la cuota de LFS está casi agotada y no se puede liberar).
...
- **Fragmentos alineados con los documentos** (`ordenar_para_fragmentos`):
  NDCG@10 de **0.206 → 0.338**, F1@3 intacto. Ver "Adoptado" en `CLAUDE.md`.
