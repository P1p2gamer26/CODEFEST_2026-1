...
def build_story() -> list:
    ...
    story.append(p(
        ...
        "la recuperacion: F1@3 0,358 contra 0,455 y NDCG@10 0,390 contra 0,516 sobre la "
        "configuracion vigente al medirlo, con 18 consultas en cero. En cambio, el "
        "grafo como clave secundaria de orden (seccion 5) es <b>neutro</b>, y se "
        "volvio a comprobar sobre la configuracion final: 34 consultas generan "
    ...
    story.append(bullets([
        ...
        "fragmentos, sumando los <b>6 mejores</b> de cada uno, sobre un pool de "
        "<b>200 candidatos</b> "
        ...
        "Combinacion de encoders por <b>suma ponderada de similitudes calibradas</b> "
        "sobre una sola lista (la cascada, sec. 3.1). Los tres cosenos NO comparten "
        "escala pese a compartir el intervalo teorico [-1, 1]: dentro del pool sus "
        "rangos medianos son 0,120 (MiniLM), 0,276 (gte) y 0,103 (e5), asi que con "
        "un peso uniforme gte pesaba mas de lo declarado. Cada senal se normaliza "
        "<b>min-max sobre el mismo pool y por consulta</b> antes de sumarse, con "
        "pesos <b>0,50</b> para gte y <b>1,00</b> para e5; RRF (k0=60) queda disponible solo cuando "
        ...
        "concentra los cupos sin riesgo de vaciar la respuesta (medido al adoptarlo: "
        "F1@3 0,440 a 0,455). El umbral 0,8 se eligio por robustez, no como argmax.",
    ...
    story.append(p(
        ...
        "dos fenomenos a la vez (q019). Cambia 6 de 50 lineas y deja el F1@3 en 0,455. "
        "El F1@3 no tiende a 1: la sec. 10.2.2 fija "
        ...
        "truth es <b>0,906</b>. Toda cifra de F1@3 hay que leerla contra ese techo, "
        "no contra 1."
    ))
    story.append(p(
    ...
    story.append(p(
        "<b>El ultimo cambio adoptado, y el que fija las cifras entregadas, es "
        "calibrar la escala de los tres encoders.</b> La cascada sumaba cosenos de "
        "tres espacios distintos como si compartieran escala, y no la comparten: "
        "sus rangos medianos dentro del pool son 0,120 (MiniLM), 0,276 (gte) y "
        "0,103 (e5), o sea que con un peso uniforme <b>gte decidia el orden mas de "
        "lo que declaraba su parametro</b>. Cada senal se normaliza min-max sobre "
        "el mismo pool y por consulta antes de sumarse, con pesos separados (gte "
        "0,50, e5 1,00); corregida la escala, el ancho del pool y el tope de la "
        "agregacion quedaban calibrados para el regimen anterior y se re-midieron "
        "en <b>200 candidatos y los 6 mejores fragmentos</b>. Resultado sobre las "
        "50 consultas: <b>F1@3 = 0,499 y NDCG@10 = 0,558</b> (0,539 penalizado) "
        "frente a 0,455 y 0,516, con las consultas sin ningun acierto bajando de "
        "<b>11 a 8</b>; el 0,499 es el <b>55% del techo de 0,906</b>. La salvedad "
        "va junto a la cifra: la ganancia se sostiene en las 41 de anotacion humana "
        "(0,486 a 0,518) pero es <b>plana en las 10 consultas sin sesgo de "
        "pooling</b> (0,433, identica), y la prueba de signos da p = 0,180. Se "
        "adopta por el criterio fijado de antemano &mdash; IC al 90% del delta "
        "pareado excluyendo una perdida de 0,02 en las tres metricas de las 50, y "
        "no aumentar las consultas en cero &mdash; no por quedar demostrado fuera "
        "de esta muestra."
    ))
    story.append(p(
        "<b>Antes de la calibracion se habian adoptado otras dos mejoras, que se "
        "componen entre si.</b> La primera fue <b>ampliar el pool</b>, que la "
        "cascada desaprovechaba: recuperaba 200 candidatos y descartaba la mayoria "
        "antes de agregar, de modo que la profundidad extra solo "
    ...
    story.append(p(
        ...
        "juntas (F1@3 / NDCG@10 sobre las 50 consultas): la configuracion previa "
        "daba <b>0,363 / 0,392</b>; solo el glosario, 0,379 / 0,403; solo el pool "
        "ampliado, 0,366 / 0,434; y <b>las dos juntas, 0,402 / 0,457</b>. Los "
        "efectos se componen: ninguna de las dos anula a la otra."
    ))
    story.append(p(
        "<b>La ronda siguiente anadio dos palancas mas, y tambien se componen.</b> "
        "Se subio el peso del re-puntuador (sec. 3.1) y el glosario gano tres "
        ...
        "0,490</b>, mejor que cualquiera por separado. La salvedad se declaro junto "
        "al numero: contra el glosario solo la ganancia en las 50 es estrecha "
        "(NDCG@10 +0,004), y el cambio gana 9 consultas y <i>pierde</i> 4."
    ))
    story.append(p(
    ...
    story.append(bullets([
        ...
        "abajo: descontando el aparato bibliografico da 0,539 frente a 0,558. El "
        ...
        "en <i>metadata.jsonl</i> con una fila sin vector (<i>en_indice</i> = false), manteniendo la alineacion indice&ndash;metadata. Los otros tres "
        "&mdash; un JSON de 0 bytes y dos HTML de error con extension <i>.pdf</i>, "
        "detectados por contenido &mdash; tampoco aportan texto.",
        "<b>Campo <i>fuente</i>:</b> el nombre de archivo estandarizado del inventario "
        "de ADL, nunca la URL (que va en un campo <i>url</i> aparte). La sec. "
        "10.2.1 sugeria <i>fuente</i> como clave de emparejamiento y ADL aclaro "
        "que es <i>doc_id</i>; el nombre funciona bajo las dos lecturas.",
